    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        @if (auth()->user()->user_role == 1)
                <button class="btn btn-primary off-canvas" type="button" data-bs-toggle="offcanvas"
                    data-bs-target="#offcanvasRight" aria-controls="offcanvasRight">Add Photo</button>
            @endif
    </div>
</div>
<!-- PAGE-HEADER END -->
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    <img alt="image" class="avatar avatar-sm br-7" src="{{ asset($gallery->image) }}">
                                </td>
                                <td>{{ $gallery->created_at }}</td>
                                <td>{{ $gallery->updated_at }}</td>
                                <td class="text-center">
                                    <x-buttons.action-pill-button iconClass="fa fa-eye" iconColor="secondary"
                                        href="{{ route('admin.gallerys.view', $gallery->id) }}" />
                                    @if (auth()->user()->user_role != 3)
                                    <x-buttons.action-pill-button
                                        href="{{ route('admin.gallerys.edit', $gallery->id) }}"
                                        iconClass="fa fa-pencil" iconColor="warning"
                                        modalTarget="editUserModal" />
                                    @endif
                                    @if (auth()->user()->user_role == 1)
                                    <x-buttons.action-pill-button
                                        href="{{ route('admin.gallerys.destroy', $gallery->id) }}"
