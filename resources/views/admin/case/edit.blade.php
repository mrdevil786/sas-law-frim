...
@section('admin-main-section')
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="page-title">{{ $isEdit ? 'Edit Case Study' : 'Add Case Study' }}</h1>
        </div>
    </div>
    <!-- PAGE-HEADER END -->
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ $isEdit ? 'Edit Case Study' : 'Add Case Study' }}</h3>
                </div>
                <div class="card-body">
                    <form method="POST"
                        action="{{ $isEdit ? route('admin.cases.update', $blog->id) : route('admin.cases.store') }}"
                        ...
                        <center>
                            <button class="btn btn-primary"
                                type="submit">{{ $isEdit ? 'Update Case Studies' : 'Add Case Studies' }}</button>
                        </center>
                    </form>
                </div>
            </div>
